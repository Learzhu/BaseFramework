...
package com.learzhu.baseframeworklibs.manager;
...
/**
 * ThreadManager.java是BaseFramework的线程管理类类。
 *
 * @author Learzhu
 * @version 1.0.0 2019-03-26 17:26
 * @use ThreadManager.getInstance().runThread(...);
 * 在使用ThreadManager的Context被销毁前ThreadManager.getInstance().destroyThread(...);
 * 在应用退出前ThreadManager.getInstance().finish(); * @update UserName 2019-03-26 17:26
 * @updateDes
 */
public class ThreadManager {
    ...
    public Handler runThread(String name, Runnable runnable) {
        if (StringUtil.isNotEmpty(name, true) == false || runnable == null) {
            LogUtil.e(TAG, "runThread  StringUtil.isNotEmpty(name, true) == false || runnable == null >> return");
            return null;
        }
